...
description: "Hands-on tutorial to connect Google Calendar, transactional email, Slack team notifications, and Harly Sign to your workspace in a single guided session."
---
...
- Choose Harly Sign or DocuSeal for offer documents
...
| Harly Sign | Nothing extra — it is built in |
...
<Steps>
  ...
  <Step title="Register the Harly webhook">
    ...
    ```text
    https://hiring.example.com/api/webhooks/email/<provider>?ws=<workspace-id>
    ```

    Use `resend` or `postmark` for `<provider>` and include the workspace
    inbound secret shown in the settings screen.
  </Step>
  <Step title="Verify">
...
Harly Sign is built in and requires no external account. Choose the offer
delivery channel in **Settings → Integrations → Harly Sign**.

<Steps>
  <Step title="Enable signing">
    Open **Settings → Integrations → Harly Sign** and select **Native
    (built-in)** under **Offer signature delivery**.
  </Step>
  <Step title="Configure sender details">
    Open **Signature settings** to enable remote links, email OTP, saved
    signatures, and the signing timeline.
  </Step>
  <Step title="Test with an offer">
    Create a test offer on a test application. Choose **Send offer**, place a
    required signature field and any text fields, then send to a test email
    address. Open the signing link, complete the required fields, and confirm
    the offer/document state updates in Harly.
  </Step>
</Steps>
...
<Check>Signing: test offer reaches its completed state from the offer record.</Check>
