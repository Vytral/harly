    ...
    const emails = await withConcurrencyRetry(
      ...
      if (changedStage) {
        ...
        stageEvents.push({
          ...
          toStageId: input.toStageId,
          status: nextStatus,
          becameHired: application.status !== "hired" && nextStatus === "hired",
          becameRejected:
        ...
        for (const applicationId of appIds) {
          ...
          if (shouldNotifyPortalStatus && (becameRejected || becameHired)) {
            await tx.insert(candidatePortalNotifications).values({
              ...
              type: becameHired ? "application_hired" : "application_rejected",
              title: becameHired
...
export async function updateApplicationStatus(
  ...
  try {
    ...
    if (applicationIds.length !== input.applicationIds.length) {
      return { success: false, error: "One or more applications were not found." };
    }
      ...
      for (const application of applicationRows) {
        ...
        if (terminalStageName) {
          ...
          if (currentStage && statusForStageName(currentStage.name) !== "active") {
            const [previousStage] = await tx
              ...
              .where(
                and(
                  eq(applicationStageHistory.workspaceId, input.workspaceId),
                  eq(applicationStageHistory.applicationId, application.id),
                  eq(applicationStageHistory.toStageId, application.currentStageId),
            ...
            targetStageId = previousStage?.fromStageId ?? application.currentStageId;
        ...
        if (stageChanged) {
          ...
          stageEvents.push({
            ...
            toStageId: targetStageId,
            status: input.status,
            becameHired: application.status !== "hired" && input.status === "hired",
            becameRejected:
              application.status !== "rejected" && input.status === "rejected",
          });
        }
        ...
        if (input.status === "rejected" && statusChanged) {
          ...
          input.status === "active" &&
          targetStage &&
          normalizeStageEmailConfig(targetStage.emailConfig).candidateUpdatesEnabled
        ) {
          collectedEmails.push({
            type: "stage",
            candidateEmail: application.candidateEmail,
