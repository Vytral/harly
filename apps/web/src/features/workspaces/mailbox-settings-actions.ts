...
export async function saveMailboxSettingsAction(
  ...
  if (!parsed.success)
    return {
      ok: false,
      error: parsed.error.issues[0]?.message ?? "Invalid mailbox settings.",
    };
  const value = parsed.data;
  const [existing] = await db
    .select()
    .from(mailboxes)
    .where(eq(mailboxes.workspaceId, context.organization.id))
    .limit(1);
  if (!existing && (!value.imapPassword || !value.smtpPassword))
    return { ok: false, error: "Add both mailbox passwords before saving." };
  ...
  const columns = {
    ...
    sentFolder: value.sentFolder || null,
    ...(imap
      ? {
          imapPasswordCiphertext: imap.ciphertext,
          imapPasswordIv: imap.iv,
          imapPasswordTag: imap.tag,
        }
      : {}),
    ...(smtp
      ? {
          smtpPasswordCiphertext: smtp.ciphertext,
          smtpPasswordIv: smtp.iv,
          smtpPasswordTag: smtp.tag,
        }
      : {}),
  };
  if (existing)
...
export async function disableMailboxSettingsAction(): Promise<MailboxActionResult> {
  ...
  await db
    .update(mailboxes)
    .set({ enabled: false, lastError: null, updatedAt: new Date() })
    .where(eq(mailboxes.workspaceId, context.organization.id));
