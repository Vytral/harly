  ...
  return {
    moveCandidateStage: tool({
      ...
      inputSchema: z.object({
        ...
        toStageId: z.string().describe("The destination stage id (from jobDetail)."),
        candidateName: z.string().describe("The candidate's full name (for the confirmation card)."),
        fromStageName: z.string().describe("The candidate's current stage name (for the card)."),
        toStageName: z.string().describe("The destination stage name (for the card)."),
      }),
    }),
    ...
    createTask: tool({
      ...
      inputSchema: z.object({
        ...
        candidateId: z.string().nullable().describe("Optional linked candidate id."),
        applicationId: z.string().nullable().describe("Optional linked application id."),
    ...
    createOffer: tool({
      ...
      inputSchema: z.object({
        summary,
        applicationId: z.string().describe("The application to make an offer on."),
        title: z.string().describe("Job title / offer title."),
        salaryAmount: z.number().nullable().describe("Base salary number, or null."),
        currency: z.string().nullable().describe("Currency code e.g. USD, or null."),
        salaryPeriod: z
          .enum(["annual", "monthly"])
          .nullable()
          .describe("Salary period, or null."),
        equity: z.string().nullable().describe("Equity description, or null."),
        startDate: z.string().nullable().describe("ISO date (YYYY-MM-DD), or null."),
        expiresAt: z.string().nullable().describe("ISO date the offer expires, or null."),
    ...
    decideOffer: tool({
      ...
      inputSchema: z.object({
        ...
        decision: z.enum(["accepted", "declined"]).describe("The candidate's decision."),
      }),
    }),
    ...
    scheduleInterview: tool({
      ...
      inputSchema: z.object({
        summary,
        candidateId: z.string().describe("The candidate id."),
        applicationId: z.string().describe("The application this interview is for."),
        type: z
        ...
        scheduledAt: z
          .string()
          .describe("Local datetime string (YYYY-MM-DDTHH:mm) or ISO timestamp."),
        durationMins: z.number().describe("Duration in minutes (5–480)."),
        interviewerId: z.string().nullable().describe("Interviewer user id, or null."),
      }),
    }),
    ...
    addToTalentPool: tool({
      ...
      inputSchema: z.object({
        ...
        reason: z.string().nullable().describe("Why they're being pooled, or null."),
      }),
    }),
    ...
    createScorecard: tool({
      ...
      inputSchema: z.object({
        ...
        stageName: z.string().nullable().describe("Stage being evaluated, or null."),
      }),
    }),
