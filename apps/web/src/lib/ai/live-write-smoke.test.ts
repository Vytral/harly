...
describe.skipIf(!live)("live Harly AI write smoke test", () => {
  it(
    "creates, updates, verifies, and cleans up a real temporary task",
    async () => {
      loadEnv({
      ...
      const [
        { db, workspaceSettings, member, user, tasks, candidates, applications, candidateTags },
        { eq, and, isNull },
      ] = await Promise.all([import("@harly/db"), import("drizzle-orm")]);
      const { confirmAgentWriteAction } = await import(
        "./agent/write-actions"
      );
      const { deleteTask } = await import("@/features/tasks/actions");
      const { updateInterview } = await import("@/features/interviews/actions");
      const { removeCandidateTag } = await import("@/features/candidates/actions");
      ...
      const scoreTool = (
        ...
        }).generateCandidateScore as unknown as {
      ...
      const evaluation = await scoreTool({
      ...
      try {
        ...
        const created = await confirmAgentWriteAction("createTask", taskInput, taskActionId);
        expect(created.success).toBe(true);
        const replayed = await confirmAgentWriteAction("createTask", taskInput, taskActionId);
        ...
        const emailResult = await confirmAgentWriteAction(
          "sendCandidateEmail",
          {
