...
describe.skipIf(!live)("live Harly AI agent read smoke test", () => {
  it("uses real workspace data for integrations, candidate context, and reports", async () => {
    loadEnv({
    ...
    expect(
      integrationTools.some(
        (call) => call.toolName === "connectedIntegrations",
      ),
    ).toBe(true);
    expect(integrations.text.length).toBeGreaterThan(20);

    const capabilities = await run(
      "Can Harly automatically publish a native job on LinkedIn? Use the real Harly capability data and explain the available alternative.",
    );
    const capabilityTools = capabilities.steps.flatMap(
      (step) => step.toolCalls ?? [],
    );
    expect(
    ...
    const distributionTools = distribution.steps.flatMap(
      (step) => step.toolCalls ?? [],
    );
    expect(
      distributionTools.some((call) => call.toolName === "resolveJob"),
    ).toBe(true);
    expect(
      distributionTools.some(
        (call) => call.toolName === "jobDistributionOptions",
      ),
    ).toBe(true);
    expect(distribution.text).toMatch(/LinkedIn|enlace|link/i);
    expect(distribution.text).toMatch(/native|nativo/i);
    ...
    const writeWasProposed = (result: {
      steps: Array<{ toolCalls?: Array<{ toolName: string }> }>;
    }) =>
      result.steps
        .flatMap((step) => step.toolCalls ?? [])
        .some((call) =>
          AGENT_WRITE_TOOLS.some((name) => name === call.toolName),
        );
    ...
    expect(ambiguity.text).toMatch(
      /which|role|application|position|candidate|email|spelling/i,
    );
    ...
    // Candidate scoring is a persisted write and must travel through the
    // same confirmation/idempotency boundary as every other agent write.
    const evaluation = await confirmAgentWriteAction(
      "generateCandidateScore",
      {
        summary: `Generate an evaluation for ${candidateName}`,
    ...
    const reportTools = report.steps.flatMap((step) => step.toolCalls ?? []);
    expect(
    ...
    expect(proposalInput.location).toBe("https://meet.google.com/wdk-sfc-xck");
