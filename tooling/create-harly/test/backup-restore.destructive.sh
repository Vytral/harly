...
#   2. Take a backup with the CLI (`create-harly backup`).
#   3. DESTROY: drop every table and delete every uploaded object.
#   4. Restore from the backup (`create-harly restore`).
...
#
# Usage (from a directory created by `create-harly init`):
...
[ -f "$CLI" ] || fail "CLI not found at $CLI (build it: pnpm --filter @harly/create build)"
