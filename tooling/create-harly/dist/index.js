...
async function upgrade() {
  ...
  if (toVersion) {
    ...
    const updated = current.replace(/^HARLY_IMAGE=.*$/m, `HARLY_IMAGE=${envLine(nextImage)}`).replace(/^HARLY_VERSION=.*$/m, `HARLY_VERSION=${envLine(toVersion)}`);
    await atomicWrite(envPath, updated, 384);
