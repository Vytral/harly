...
const releaseManifestUrl = process.env.HARLY_RELEASE_MANIFEST_URL ?? "https://raw.githubusercontent.com/Vytral/harly/main/release-manifest.json";
...
async function officialRelease(): Promise<HarlyRelease> {
  ...
  try {
    const response = await fetch(releaseManifestUrl, { signal: AbortSignal.timeout(2_000) });
...
x-env: &env
  ...
  DOCUSIGN_BASE_URL: \${DOCUSIGN_BASE_URL:-https://account-d.docusign.com}
...
const envExampleWithDocuSign = `${envExample}# Optional DocuSign global fallback\nDOCUSIGN_BASE_URL=https://account-d.docusign.com\nDOCUSIGN_CLIENT_ID=\nDOCUSIGN_CLIENT_SECRET=\n`;
...
async function init() {
  ...
  try {
    ...
    const templates: Array<[string, string, number?]> = [
      ...
      [".env.example", envExampleWithDocuSign],
      [".gitignore", ".env\nbackups/\n"],
...
async function launch(explicitDirectory?: string, confirmed = false) {
  ...
  if (interactive && !confirmed) {
    p.outro(`Harly is ready at ${pc.cyan(config.publicUrl)} · run ${pc.cyan("npx @harly/cli doctor .")}`);
  } else if (!interactive) {
...
async function cloudGuide(provider: "railway" | "fly" | "digitalocean") {
  ...
  const env = [
    ...
    `HARLY_SETUP_SECRET=${envLine(runtimeSecrets.setup)}`, "DOCUSIGN_BASE_URL=https://account-d.docusign.com", "DOCUSIGN_CLIENT_ID=", "DOCUSIGN_CLIENT_SECRET=", "STORAGE_PROVIDER=\"s3\"",
...
`);
  ...
  if (provider === "digitalocean") {
    ...
    const appSpec = [
      ...
      publicEnv("DOCUSIGN_BASE_URL", "https://account-d.docusign.com"),
      secretEnv("DOCUSIGN_CLIENT_ID", ""), secretEnv("DOCUSIGN_CLIENT_SECRET", ""),
      ...
      `services:`, "  - name: web", `    image: { registry_type: GHCR, registry: vytral, repository: harly, tag: ${deploymentRelease.version} }`,
      ...
      "workers:", "  - name: scheduler", `    image: { registry_type: GHCR, registry: vytral, repository: harly, tag: ${deploymentRelease.version} }`,
      "    run_command: node /app/runtime.mjs scheduler", "    instance_count: 1", "    instance_size_slug: apps-s-1vcpu-0.5gb",
      "jobs:", "  - name: migrate", "    kind: PRE_DEPLOY", `    image: { registry_type: GHCR, registry: vytral, repository: harly, tag: ${deploymentRelease.version} }`,
...
async function main() {
  switch (command) {
    ...
    case "--version": case "-v": process.stdout.write(`${cliVersion}\n`); return;
