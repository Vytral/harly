...
test("DigitalOcean assets keep the database secret app-wide and run all runtime roles", async () => {
  ...
  assert.match(source, /import \{ embeddedRelease, releaseImage, type HarlyRelease \} from "\.\/release\.js"/);
  assert.match(source, /https:\/\/raw\.githubusercontent\.com\/Vytral\/harly\/main\/release-manifest\.json/);
...
test("release manifest is the single source of truth for deployment assets", async () => {
  const [manifestContents, fly, railwayReadme, releaseModule, ci] = await Promise.all([
    readFile(path.join(repositoryRoot, "release-manifest.json"), "utf8"),
    readFile(path.join(repositoryRoot, "fly.toml"), "utf8"),
    readFile(path.join(repositoryRoot, "deploy/railway/README.md"), "utf8"),
  ...
  assert.match(fly, new RegExp(digestImage.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")));
  assert.match(railwayReadme, new RegExp(tagImage.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")));
...
test("cloud documentation links and provider instructions resolve", async () => {
  ...
  assert.match(readme, /Deploy on Fly\.io/);
