...
const digitalOceanPath = path.join(root, "deploy/digitalocean/app.template.yaml");
const railwayReadmePath = path.join(root, "deploy/railway/README.md");
...
if (args.some((arg) => !["--check", "--version", "--digest", versionArg, digestArg].includes(arg))) {
  throw new Error("Usage: sync-release-assets.mjs [--check] [--version X.Y.Z --digest sha256:…]");
}
if (Boolean(versionArg) !== Boolean(digestArg)) {
  throw new Error("--version and --digest must be supplied together.");
}
if (versionArg && !/^\d+\.\d+\.\d+(?:-[0-9A-Za-z.-]+)?(?:\+[0-9A-Za-z.-]+)?$/.test(versionArg)) {
  throw new Error("Release version must be a SemVer value without a leading v.");
}
...
if (manifest.schemaVersion !== 1 || typeof manifest.image !== "string" || !/^ghcr\.io\/vytral\/harly$/.test(manifest.image)) {
  throw new Error("release-manifest.json must describe the official ghcr.io/vytral/harly image.");
}
if (!/^\d+\.\d+\.\d+(?:-[0-9A-Za-z.-]+)?(?:\+[0-9A-Za-z.-]+)?$/.test(manifest.version) || !/^sha256:[a-f0-9]{64}$/.test(manifest.digest)) {
...
const digitalOcean = (await readFile(digitalOceanPath, "utf8")).replace(/^(\s*tag:)\s*[^\n]+$/m, `$1 ${manifest.version}`);
const railwayReadme = (await readFile(railwayReadmePath, "utf8")).replace(/ghcr\.io\/vytral\/harly:[^`;\s]+/, tagImage);

const outputs = new Map([
  [manifestPath, `${JSON.stringify(manifest, null, 2)}\n`],
  [flyPath, fly],
  [digitalOceanPath, digitalOcean],
  [railwayReadmePath, railwayReadme],
...
for (const [file, expected] of outputs) {
  ...
  else process.stderr.write(`${path.relative(root, file)} is out of sync with release-manifest.json\n`);
}
