...
export const documents = pgTable(
  "documents",
  {
    ...
    // Stable effect identity for documents generated by an automation. A
    // retry must return the same document instead of creating another PDF.
    workflowEffectId: text("workflow_effect_id"),
  ...
  (table) => [
    ...
    uniqueIndex("documents_workspace_workflow_effect_uidx").on(
      table.workspaceId,
      table.workflowEffectId,
    ),
...
// A durable package is the identity of one workflow request_documents action.
// Keeping it separate from the individual rows prevents a document wait from
// accidentally observing unrelated requests on the same application.
export const documentRequestPackages = pgTable(
  "document_request_packages",
  {
    ...
    candidateId: uuid("candidate_id")
      .notNull()
      .references(() => candidates.id, { onDelete: "cascade" }),
    // One workflow effect creates one package. Retries reuse this exact key.
    workflowEffectId: text("workflow_effect_id"),
  ...
  (table) => [
    index("document_request_packages_workspace_application_idx").on(
      table.workspaceId,
      table.applicationId,
      table.createdAt,
    ),
    index("document_request_packages_workspace_status_idx").on(
      table.workspaceId,
      table.status,
      table.dueAt,
    ),
    uniqueIndex("document_request_packages_workspace_effect_uidx").on(
      table.workspaceId,
      table.workflowEffectId,
    ),
    check("document_request_packages_version_check", sql`${table.version} > 0`),
    check(
      "document_request_packages_status_check",
      sql`${table.status} in ('pending', 'completed', 'declined', 'expired', 'cancelled')`,
    ),
  ],
);
...
export const documentRequests = pgTable(
  "document_requests",
  {
    ...
    applicationId: uuid("application_id")
      .notNull()
      .references(() => applications.id, { onDelete: "cascade" }),
    packageId: uuid("package_id").references(() => documentRequestPackages.id, {
      onDelete: "cascade",
    }),
    candidateId: uuid("candidate_id")
      .notNull()
    ...
    // Stable workflow effect identity. A retry of the same node must return
    // the existing request package instead of creating duplicate portal work.
    workflowEffectId: text("workflow_effect_id"),
    requestedById: text("requested_by_id").references(() => user.id, {
      onDelete: "set null",
    }),
  ...
  (table) => [
    ...
    index("document_requests_package_idx").on(table.packageId),
    uniqueIndex("document_requests_workspace_effect_uidx").on(
      table.workspaceId,
      table.workflowEffectId,
    ),
  ],
...
export type DocumentRequestPackage =
  typeof documentRequestPackages.$inferSelect;
export type NewDocumentRequestPackage =
  typeof documentRequestPackages.$inferInsert;
...
export const offers = pgTable(
  "offers",
  {
    ...
    createdById: text("created_by_id")
      .notNull()
      .references(() => user.id, { onDelete: "restrict" }),
    /** Stable idempotency identity for a create_offer workflow effect. */
    workflowEffectId: text("workflow_effect_id"),
    decidedAt: timestamp("decided_at", { withTimezone: true }),
  ...
  (table) => [
    ...
    uniqueIndex("offers_workspace_workflow_effect_uidx").on(
      table.workspaceId,
      table.workflowEffectId,
    ),
  ],
...
export const signatureEnvelopes = pgTable(
  "signature_envelopes",
  {
    ...
    // Stable workflow effect identity for idempotent signature invitations.
    workflowEffectId: text("workflow_effect_id"),
    createdById: text("created_by_id").references(() => user.id, {
      onDelete: "set null",
    }),
  ...
  (table) => [
    ...
    uniqueIndex("signature_envelopes_workspace_effect_uidx").on(
      table.workspaceId,
      table.workflowEffectId,
    ),
...
export const domainEventOutbox = pgTable(
  ...
  (table) => [
    ...
    index("domain_event_outbox_workspace_event_id_idx").on(
      table.workspaceId,
      table.eventName,
      table.id,
    ),
...
// Reusable document templates for Automations v2. The body is sanitized by
// the document-template service before it is stored; the runtime snapshots
// the selected content into the generated PDF, so later edits cannot change
// an already-created document.
export const workflowDocumentTemplates = pgTable(
  "workflow_document_templates",
  {
    ...
    title: text("title").notNull(),
    body: text("body").notNull(),
    format: text("format").default("rich_text").notNull(),
  ...
  (table) => [
    uniqueIndex("workflow_document_templates_workspace_name_idx").on(
      table.workspaceId,
      sql`lower(${table.name})`,
    ),
    index("workflow_document_templates_workspace_updated_idx").on(
      table.workspaceId,
      table.updatedAt,
    ),
    index("workflow_document_templates_workspace_active_idx").on(
      table.workspaceId,
      table.archivedAt,
    ),
  ],
...
export type WorkflowDocumentTemplate =
  typeof workflowDocumentTemplates.$inferSelect;
export type NewWorkflowDocumentTemplate =
  typeof workflowDocumentTemplates.$inferInsert;
...
export const interviews = pgTable(
  "interviews",
  {
    ...
    // Stable workflow effect identity. Retries must return the same meeting,
    // never schedule a second slot for the candidate or interviewer.
    workflowEffectId: text("workflow_effect_id"),
  ...
  (table) => [
    ...
    uniqueIndex("interviews_workspace_workflow_effect_uidx").on(
      table.workspaceId,
      table.workflowEffectId,
    ),
  ],
...
// user within a workspace. Messages store a bounded, redacted AI SDK
// UIMessage `parts` array so credentials and authorization headers are never
// retained verbatim.
export const aiConversations = pgTable(
  "ai_conversations",
  {
    ...
    lastMessageAt: timestamp("last_message_at", { withTimezone: true })
      .defaultNow()
      .notNull(),
    // Enforced on reads and by the scheduled reaper; activity extends the
    // window without making a conversation indefinite.
  ...
  (table) => [
    ...
    index("ai_conversations_retention_idx").on(table.retentionUntil),
  ],
);
...
export const aiMessages = pgTable(
  "ai_messages",
  {
    ...
    // Redacted AI SDK UIMessage.parts[]; this version is auditable when the
    // sanitizer evolves without retaining the sensitive original.
    parts: jsonb("parts")
      .default(sql`'[]'::jsonb`)
      .notNull(),
...
/**
 * Many-to-many erasure linkage between a conversation and every candidate
 * referenced in it, not only the single "active" candidate stored on
 * `aiConversations.candidateId`. A candidate can be pulled into a turn via
 * the @mention picker or by the model resolving/returning them through a
 * read tool result; either way their PII may end up embedded in
 * `aiMessages.parts`, so a GDPR Art. 17 erasure (IA-02 / AI15) must reach
 * this row set too, not just the conversation whose `candidateId` happens to
 * match. `ON DELETE CASCADE` on both sides means deleting either the
 * conversation or the candidate removes the link automatically; explicit
 * cleanup in `deleteConversationsForCandidate` stays as defense in depth.
 */
export const aiConversationCandidates = pgTable(
  "ai_conversation_candidates",
  {
    id: uuid("id").defaultRandom().primaryKey(),
    conversationId: uuid("conversation_id")
      .notNull()
  ...
  (table) => [
    uniqueIndex("ai_conversation_candidates_pair_uidx").on(
      table.conversationId,
      table.candidateId,
    ),
    index("ai_conversation_candidates_candidate_idx").on(table.candidateId),
  ],
);
...
export const workflowDefinitions = pgTable(
  "workflow_definitions",
  {
    ...
    // v2: live runs keep using trigger/conditions/actions until a new version
    // is published. Drafts live in workflow_drafts.
    engineVersion: integer("engine_version").default(1).notNull(),
    publishedVersionId: uuid("published_version_id"),
    triggerGeneration: integer("trigger_generation").default(0).notNull(),
    archivedAt: timestamp("archived_at", { withTimezone: true }),
    ...timestamps(),
  },
  (table) => [
    ...
    index("workflow_definitions_published_version_idx").on(
...
/**
 * Authenticated inbound trigger endpoint owned by one published workflow.
 * The URL token is stored only as a hash; the HMAC secret is encrypted at
 * rest and returned once when the endpoint is created. Keeping this separate
 * from outbound webhook_endpoints prevents delivery configuration from being
 * confused with an ingress capability.
 */
export const workflowWebhookEndpoints = pgTable(
  "workflow_webhook_endpoints",
  {
    ...
    secretIv: text("secret_iv").notNull(),
    secretTag: text("secret_tag").notNull(),
    // Draft 7 JSON Schema enforced on inbound payloads before a receipt/outbox
    // event is committed. The empty object keeps the default permissive.
  ...
  (table) => [
    uniqueIndex("workflow_webhook_endpoints_workspace_token_uidx").on(
      table.workspaceId,
      table.tokenHash,
    ),
    index("workflow_webhook_endpoints_workspace_workflow_idx").on(
      table.workspaceId,
      table.workflowId,
      table.enabled,
    ),
...
/** One receipt per endpoint and external event identity. */
export const workflowWebhookReceipts = pgTable(
  "workflow_webhook_receipts",
  {
    ...
    endpointId: uuid("endpoint_id")
      .notNull()
      .references(() => workflowWebhookEndpoints.id, { onDelete: "cascade" }),
  ...
  (table) => [
    uniqueIndex("workflow_webhook_receipts_endpoint_event_uidx").on(
      table.endpointId,
      table.externalEventId,
    ),
    index("workflow_webhook_receipts_workspace_received_idx").on(
      table.workspaceId,
      table.receivedAt,
    ),
  ],
...
export const workflowDefinitionVersions = pgTable(
  "workflow_definition_versions",
  {
    ...
    graph: jsonb("graph")
      .default(sql`'{}'::jsonb`)
      .notNull(),
    compilerVersion: integer("compiler_version").default(0).notNull(),
    contentHash: text("content_hash").default("").notNull(),
    layoutSnapshot: jsonb("layout_snapshot")
      .default(sql`'{"positions":{},"collapsedNodeIds":[]}'::jsonb`)
      .notNull(),
    createdAt: timestamp("created_at", { withTimezone: true })
      .defaultNow()
      .notNull(),
  ...
  (table) => [
    ...
    index("workflow_definition_versions_content_hash_idx").on(
      table.workspaceId,
      table.contentHash,
    ),
  ],
...
/** Editable working copy. Saving never mutates the published snapshot. */
export const workflowDrafts = pgTable(
  ...
  (table) => [
    uniqueIndex("workflow_drafts_workflow_uidx").on(table.workflowId),
    index("workflow_drafts_workspace_updated_idx").on(
      table.workspaceId,
      table.updatedAt,
    ),
  ],
...
/**
 * Server-generated automation changes proposed by Harly AI. A proposal is a
 * bounded, reviewable artifact: it is not a workflow draft and cannot publish
 * or invoke an automation handler. The base revision/hash make stale chat and
 * editor responses fail closed when the user keeps editing.
 */
export const automationAiProposals = pgTable(
  "automation_ai_proposals",
  {
    id: uuid("id").defaultRandom().primaryKey(),
    workspaceId: text("workspace_id")
      .notNull()
    ...
    /** Hash of the local Builder snapshot used to author the proposal. */
    localSnapshotHash: text("local_snapshot_hash"),
    name: text("name").notNull(),
    description: text("description"),
    graph: jsonb("graph").notNull(),
    layout: jsonb("layout").notNull(),
    /** Control-plane limits proposed alongside, but never embedded in, graph JSON. */
    operationalPolicy: jsonb("operational_policy"),
    /** Stable, reviewable diff calculated against the verified base snapshot. */
    diff: jsonb("diff")
      .default(sql`'[]'::jsonb`)
      .notNull(),
    validationIssues: jsonb("validation_issues")
    ...
    /** Non-secret redaction/audit facts for retained proposal payloads. */
    redactionMetadata: jsonb("redaction_metadata")
      .default(sql`'{"version":1,"secretsRedacted":true}'::jsonb`)
      .notNull(),
    // prepared | applying | applied | expired | rejected
    status: text("status").default("prepared").notNull(),
    applyActionId: text("apply_action_id"),
    appliedRevision: integer("applied_revision"),
    expiresAt: timestamp("expires_at", { withTimezone: true }).notNull(),
    ...timestamps(),
  },
  (table) => [
    index("automation_ai_proposals_workspace_actor_idx").on(
      table.workspaceId,
      table.actorId,
      table.createdAt,
    ),
    index("automation_ai_proposals_workflow_idx").on(
      table.workspaceId,
      table.workflowId,
      table.createdAt,
    ),
    uniqueIndex("automation_ai_proposals_workspace_action_uidx").on(
      table.workspaceId,
      table.applyActionId,
    ),
    index("automation_ai_proposals_expires_idx").on(table.expiresAt),
  ],
);

/**
 * Durable, workspace-scoped jobs for AI automation work that must outlive an
 * HTTP request. Jobs are deliberately generic so long simulations, future
 * provider-backed dry runs, and resumable build phases share one lease and
 * retry contract. Payloads/results are bounded by the application layer and
 * must never contain provider credentials.
 */
export const automationAiJobs = pgTable(
  "automation_ai_jobs",
  {
    id: uuid("id").defaultRandom().primaryKey(),
    workspaceId: text("workspace_id")
  ...
  (table) => [
    uniqueIndex("automation_ai_jobs_workspace_idempotency_uidx").on(
      table.workspaceId,
      table.idempotencyKey,
    ),
    index("automation_ai_jobs_queue_idx").on(
      table.status,
      table.availableAt,
      table.leaseUntil,
    ),
    index("automation_ai_jobs_workspace_created_idx").on(
      table.workspaceId,
      table.createdAt,
    ),
  ],
...
export const workflowRuns = pgTable(
  "workflow_runs",
  {
    ...
    // v2 uses a graph cursor; legacy step indexes/status remain untouched.
    engineVersion: integer("engine_version").default(1).notNull(),
    versionId: uuid("version_id").references(
      () => workflowDefinitionVersions.id,
    ),
    logicalStatus: text("logical_status"),
    cursorNodeId: text("cursor_node_id"),
    // Set while a retry is waiting. It survives worker replacement so the
    // next lease can request the failed action again without deleting evidence.
    ...
    // Every event chain has a stable root even after its first run finishes.
    // It makes loop protection and the cross-workflow effect budget enforceable
    // without walking an unbounded parent hierarchy at dispatch time.
    rootRunId: uuid("root_run_id"),
    lineageDepth: integer("lineage_depth").default(0).notNull(),
    lineageExternalActions: integer("lineage_external_actions")
      .default(0)
      .notNull(),
    error: text("error"),
    ...timestamps(),
  ...
  (table) => [
    // Required for composite FKs (workspace_id, id) from child tables.
    uniqueIndex("workflow_runs_workspace_id_uidx").on(
      table.workspaceId,
      table.id,
    ),
    ...
    index("workflow_runs_root_idx").on(table.workspaceId, table.rootRunId),
    index("workflow_runs_v2_queue_idx").on(
      table.engineVersion,
      table.logicalStatus,
      table.nextAttemptAt,
      table.leaseUntil,
    ...
    check(
      "workflow_runs_engine_version_check",
      sql`${table.engineVersion} in (1, 2)`,
    ),
    check(
      "workflow_runs_v2_state_check",
      sql`${table.engineVersion} = 1 or (${table.versionId} is not null and ${table.logicalStatus} is not null and ${table.logicalStatus} in ('queued', 'running', 'waiting', 'retrying', 'succeeded', 'completed_with_warnings', 'stopped', 'failed', 'uncertain', 'cancelled'))`,
    ),
...
/**
 * One row per workflow and minute. Reservations are updated atomically before
 * external I/O, so concurrent workers cannot all observe the same stale count.
 */
export const workflowRunBuckets = pgTable(
  "workflow_run_buckets",
  {
    id: uuid("id").defaultRandom().primaryKey(),
    workspaceId: text("workspace_id")
      .notNull()
  ...
  (table) => [
    uniqueIndex("workflow_run_buckets_window_uidx").on(
      table.workspaceId,
      table.workflowId,
      table.bucketStart,
    ),
    index("workflow_run_buckets_cleanup_idx").on(table.bucketStart),
    check("workflow_run_buckets_reserved_check", sql`${table.reserved} >= 0`),
  ],
);
...
export const workflowExternalActionBuckets = pgTable(
  ...
  (table) => [
    uniqueIndex("workflow_external_action_buckets_window_uidx").on(
      table.workspaceId,
      table.workflowId,
      table.bucketStart,
    ),
 ...
 */
export const workflowNodeExecutions = pgTable(
  "workflow_node_executions",
  {
    ...
    output: jsonb("output"),
    resolvedPort: text("resolved_port"),
    errorCode: text("error_code"),
    // Structured, redacted execution diagnostics for the inspector and AI
    // repair proposals. Provider response bodies and secrets never belong here.
    errorDetails: jsonb("error_details"),
    retryable: boolean("retryable").default(false).notNull(),
    waitingKind: text("waiting_kind"),
    waitingEventName: text("waiting_event_name"),
    // Outbox identity high-water mark captured when an event wait is
    // registered. It lets the scheduler recover an event committed in the
    // small window around wait registration without replaying older events.
    waitingEventCursor: bigint("waiting_event_cursor", { mode: "number" }),
    waitingResourceId: text("waiting_resource_id"),
    // Explicitly distinguishes a document-request package from a signed
    // document. Both are resolved through the document wait path, but their
    // state lives in different tables and must not be inferred from a UUID.
    waitingResourceType: text("waiting_resource_type"),
  ...
  (table) => [
    // Composite unique must exist before child FKs (workspace_id, id). Keep
    // this constraint early so generate / hand-order SQL can reference it.
    uniqueIndex("workflow_node_executions_workspace_id_uidx").on(
      table.workspaceId,
      table.id,
    ),
    foreignKey({
    ...
    uniqueIndex("workflow_node_executions_run_node_uidx").on(
      table.runId,
      table.nodeId,
    ),
    uniqueIndex("workflow_node_executions_effect_uidx").on(table.effectKey),
    index("workflow_node_executions_event_wait_idx")
      .on(table.workspaceId, table.waitingEventName, table.waitingEventCursor)
      .where(
        sql`${table.status} = 'waiting' and ${table.waitingKind} = 'event'`,
      ),
    check(
      "workflow_node_executions_status_check",
      sql`${table.status} in ('pending', 'running', 'waiting', 'succeeded', 'failed', 'skipped', 'cancelled', 'uncertain')`,
    ),
  ],
);

/** Attempt evidence is separate from the stable node/effect identity. */
export const workflowNodeAttempts = pgTable(
  ...
  (table) => [
    foreignKey({
      columns: [table.workspaceId, table.executionId],
      foreignColumns: [
        workflowNodeExecutions.workspaceId,
        workflowNodeExecutions.id,
      ],
    }).onDelete("cascade"),
    uniqueIndex("workflow_node_attempts_execution_attempt_uidx").on(
      table.executionId,
      table.attemptNo,
    ),
    check(
      "workflow_node_attempts_number_check",
      sql`${table.attemptNo} > 0 and ${table.fenceToken} > 0`,
    ),
    check(
      "workflow_node_attempts_status_check",
      sql`${table.status} in ('running', 'waiting', 'succeeded', 'failed', 'uncertain', 'cancelled')`,
    ),
  ],
);

/** One vote per eligible actor for a durable approval node. */
export const workflowApprovalVotes = pgTable(
  ...
  (table) => [
    ...
    uniqueIndex("workflow_approval_votes_execution_actor_uidx").on(
      table.executionId,
      table.actorId,
    ),
    index("workflow_approval_votes_workspace_execution_idx").on(
      table.workspaceId,
      table.executionId,
    ),
    check(
      "workflow_approval_votes_decision_check",
...
export type WorkflowWebhookEndpoint =
  typeof workflowWebhookEndpoints.$inferSelect;
export type NewWorkflowWebhookEndpoint =
  typeof workflowWebhookEndpoints.$inferInsert;
export type WorkflowWebhookReceipt =
  typeof workflowWebhookReceipts.$inferSelect;
export type NewWorkflowWebhookReceipt =
  typeof workflowWebhookReceipts.$inferInsert;
...
export type NewWorkflowNodeExecution =
  typeof workflowNodeExecutions.$inferInsert;
export type WorkflowNodeAttempt = typeof workflowNodeAttempts.$inferSelect;
export type NewWorkflowNodeAttempt = typeof workflowNodeAttempts.$inferInsert;
export type WorkflowApprovalVote = typeof workflowApprovalVotes.$inferSelect;
export type NewWorkflowApprovalVote = typeof workflowApprovalVotes.$inferInsert;
