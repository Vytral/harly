...
try {
  const [c] = await sql`select id, deleted_at from candidates where id = ${ID}`;
  console.log("Sofía deleted_at:", c.deleted_at);
  const notes = await sql`select id, body from candidate_notes where candidate_id = ${ID}`;
  console.log(`Notes (${notes.length}):`, notes.map(n=>({id:n.id, body:n.body})));
  ...
  const apps = await sql`select id, status from applications where candidate_id = ${ID}`;
  console.log(`Applications (${apps.length}):`, apps.map(a=>({id:a.id, status:a.status})));
